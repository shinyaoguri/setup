...
被覆テストが縛るのは「実体 → 台帳」の索引だけで、**`claude/CLAUDE.md` の記述が全部どれかの意図に載っているか**までは見ていない (安定したアンカーが無い)。CLAUDE.md を通読し、台帳のどの意図にも属さない規約を次の 3 つに分ける:

- **本体の機能と代替関係にある** (本体に何かが入れば要らなくなる) → 意図として台帳へ足す。`sunset` を判定できる形で書けることが目印
- **本体の既定の振る舞いや設定で、もう足りている** → 記述ごと消す候補として Issue に挙げる
- **仕事の流儀** (コミットの書式・ブランチ運用・テストの作法など。本体に何が入っても置き換わらない) → **台帳には載せない**。`sunset` を書こうとすると「本体がこの規約を既定にしたら」のような判定できない条件になり、`depends_on` に書ける仕様も無いので、突き合わせても何も当たらず件数だけが増える (setup#246)。ここでは、記述が実態とずれていないかだけを見る

もう 1 つ見るのは、**フックや道具が自分で言うことを CLAUDE.md が重ねて書いていないか**。フックの差し戻しの文面や `--help` に在る運用の細部は、要る場面でだけ読まれれば足りる。CLAUDE.md は常時コンテキストに載るので、消せる行は消す側に倒す。
